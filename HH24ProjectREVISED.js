...
class Pioneer {
  ...
  const game = {
    ...
    food: 100, //food
    days: 0, //
    distance: 0, //semester progress
    health: 100, //wellness
    healthStatus: "Good", //grades
    wagon: {
      health: 100 //connected to healthstatus (grades)
    },
    playerTimer: 0,
    currentTime: 0,
    speed: "Stopped", //study pace
    total: 0,
    startGame: function(pName) {
      ...
      const $distanceTracker = $(
        "<h2 id='distance'>Semester Hours " + this.distance + "</h2>"
      );
      const $healthTracker = $(
        "<h2 id='healthStatus'>Grades: " + this.healthStatus + "</h2>"
      );



      const $hpTracker = $("<h2 id='hp'>Wellness: " + this.health + "</h2>");
      ...
      $(".cowswalking").append(
        $("<img id='cows' src='https://f2.toyhou.se/file/f2-toyhou-se/images/89202622_nlDTnSF2sYMiWsW.gif'>")
      );
    ...
    displayStats: function() {
      const $message = $(".message-box");
      $message.text(
        `You currently have $${this.money}, your wellness is at ${this.health}, and your gades are this well: ${this.wagon.health}. You are new to Wild Wild Aggieland, so it may take time to get used to Wild West Campus. `
      );
    ...
    statsChanger: function(speed) {
      ...
      let food = this.food;
      if (this.speed === "All Nighters" && this.food > 0) {
        this.wagon.health -= 0.4;
        this.food = food - 0.8;
        $("#food").text(`Food: ${this.food.toFixed(1)}`);
        this.distance = distance + 0.4;
        $("#distance").text(
          `Semester Progress: ${this.distance.toFixed(1)} hours`
        );
      } else if (this.speed === "Locked In" && this.food > 0) {
      ...
      } else if (this.speed === "Normal pace" && this.food > 0) {
    ...
    hunt: function() {
      ...
      $(".hunt").prepend(
        $(
          "<h1 id='huntprompt'>In Wild Wild Aggieland, sometimes I gotta find my own food! Quick, catch that rabbit (click them) before it gets dark!</h1>"
        )
      );
      $(".hunt-image").append(
        $(
          "<img id='hunting-background' src='https://f2.toyhou.se/file/f2-toyhou-se/images/89201243_HE15mlYHXA8Yia2.png'>"
        )
      );
      $(".bunnyimg").append(
    ...
    interact: function(num) {
      if (num === 1) {
        ...
        $riverDiv.append(
          $(
            "<img id='riverImage' src='https://f2.toyhou.se/file/f2-toyhou-se/images/89200461_x9OD15VQKVvh0RR.png3'>"
          )
        );
        $riverButtons.append(//???
          $(
            "<button id=longRoute>Study Solo</button>" //(adds on 2 additional days) take long route
          )
        );
        $riverButtons.append(
    ...
    interactionOptionsRiver: function(answer) {
      ...
      if (answer === "cross") {
        ...
        $(".riverInfo").prepend(
          $(
            "<h1>You decide to study solo, but you do not understand all the cowboy material well since you did not ask for help.</h1>"
          )
        );
        ...
        $(".riverInfo").prepend(
          $(
            "<h1>You decided study 1 week before the exam. You've missed out on fun org activities, but you do well on the cowboy exam.</h1>"
          )
        );
        $("#riverImage").attr("src", "https://f2.toyhou.se/file/f2-toyhou-se/images/89200461_x9OD15VQKVvh0RR.png");
        //this.days += 2;
        if (this.food > 10) {
          this.food -= 15;
        } else {
  ...
  $("body").on("click", "#yesStart", function() {
    ...
    function makeButtons() {
      $walkButtons.append($("<button id='run-button'>All Nighter</button>"));
      $walkButtons.append($("<button id='walk-button'>Locked In</button>"));
      $walkButtons.append($("<button id='stroll-button'>Normal Pace</button>"));
      $walkButtons.append(
        $("<button id='stop-button'>Slow but self care's important (Gain HP)</button>")
      );
    }
